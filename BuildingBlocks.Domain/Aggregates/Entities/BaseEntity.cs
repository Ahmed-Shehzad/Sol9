...
public abstract class BaseEntity(Ulid id) : IBaseEntity, IAuditInfo
{
    ...
    /// <summary>
    /// Gets the row version (timestamp) used for optimistic concurrency control.
    /// This is a sequential number that is incremented each time the record is updated.
    /// </summary>
    /// <remarks>
    /// The row version is automatically managed by the database and serves as a concurrency token
    /// to detect and prevent concurrent modifications to the same record.
    /// </remarks>
    [Timestamp]
