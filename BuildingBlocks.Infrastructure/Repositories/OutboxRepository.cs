...
public class OutboxRepository<TContext>(BaseDbContext<TContext> context)
    : Repository<Outbox>(context), IOutboxRepository where TContext : DbContext
{
    public async Task<List<Outbox>> FindAllProcessedMessagesAsync(CancellationToken cancellationToken = default)
    {
        ...
        var messages = await context.OutboxMessages.IgnoreQueryFilters().Where(m => m.Processed).AsQueryable()
